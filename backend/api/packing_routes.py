...
@packing_bp.route('/api/packing/days-in-month', methods=['GET'])
@login_required
def get_days_in_month():
    """Trả về danh sách ngày (1-31) có dữ liệu Packing trong tháng/năm"""
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT DISTINCT CAST([Ngày packing] AS TEXT) as ngay_str
        FROM Packing
        WHERE [Đã xóa] = 0
          AND CAST([Ngày packing] AS TEXT) LIKE ?
        ORDER BY ngay_str
    """, (month_str + '%',))
